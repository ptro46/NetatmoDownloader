...
NetatmoGetModuleMeasuresWS::NetatmoGetModuleMeasuresWS(QString token,
    ...
    :AbstractWS(),m_token(token),m_startDate(startDate),m_deviceId(deviceId),m_moduleId(moduleId),m_pModuleDelegate(delegate)
{
...
NetatmoGetModuleMeasuresWS::getRequestContent() const {
    ...
    requestContent += QString::number(m_startDate + MODULE_MEASURE_INTERVAL) ;
    requestContent += "&optimize=false&real_time=true";

    QDateTime dStart = QDateTime::fromTime_t(m_startDate);
    QDateTime dEnd   = QDateTime::fromTime_t(m_startDate + MODULE_MEASURE_INTERVAL);

    /*
    qDebug() << "POST " << requestContent ;
    qDebug() << "From : " << m_startDate << "(" << dStart.toString("yyyy-MM-dd hh:mm:ss") << ")" ;
    qDebug() << "To   : " << (m_startDate + MODULE_MEASURE_INTERVAL) << "(" << dEnd.toString("yyyy-MM-dd hh:mm:ss") << ")" ;
    */
